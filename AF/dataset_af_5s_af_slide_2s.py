...
class CinCDataset(Dataset):
    def __init__(self, split, mode, csv):
        self.block_len = 5 * 500
        self.slide_len = 2 * 500
        def seg_data_to_5s(ecg):
            ecg_5s = []

            ecg_len = ecg.shape[1]
            if ecg_len % self.block_len == 0 :
                block_num = int(ecg_len / self.block_len)
            else:
                block_num = int(ecg_len / self.block_len) + 1

            for i in range(block_num):
                if i == block_num - 1 :
                    ecg_5s.append(ecg[:, -self.block_len : ])
                else:
                    ecg_5s.append(ecg[:, i * self.block_len : (i + 1) * self.block_len])

            ecg_5s = np.stack(ecg_5s)

            return ecg_5s

        def slide_data_to_5s(ecg):
            ecg_5s = []
            ecg_len = ecg.shape[1]
            i = 0
            while i * self.slide_len + self.block_len < ecg_len :
                ecg_5s.append(ecg[:, i * self.slide_len : i * self.slide_len + self.block_len])
        ...
        for i, r in enumerate(self.Recording):
            ...
            f = self.First_label[i]
            s = self.Second_label[i]
            t = self.Third_label[i]
            af_index = class_map.index(class_map[0])
            if (f == af_index or s == af_index or t == af_index) and mode == 'train':
                temp_ecg = slide_data_to_5s(ecg)
            else:
...
def run_check_Dataset():
    ...
    a = 0
    for t, (input, truth, infor) in enumerate(dataset):
        print(truth)
        # a += len(infor)
        # print(len(infor))
        print(infor.ecg_id)
