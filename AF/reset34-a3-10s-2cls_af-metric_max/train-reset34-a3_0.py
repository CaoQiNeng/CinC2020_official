...
def do_valid(net, valid_loader, out_dir=None):
    ...
    b = np.argmax(valid_predict, axis=0)
    for i, index in enumerate(b):
        valid_predict[i][index] = 1

    accuracy, f_measure, f_beta, g_beta = compute_beta_score(valid_truth, valid_predict, 2, valid_truth.shape[1], check_errors=True)
    valid_precision, valid_recall = metric(valid_truth, (valid_predict).astype(int))
...
def run_train():
    train_fold = 3
    valid_fold = 0
    out_dir = ROOT_PATH + '/CinC2020_official_logs/result-reset34-a%d_%d-10s-2cls_af-metric_max'%(train_fold, valid_fold)
